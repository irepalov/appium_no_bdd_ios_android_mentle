...
public class AndroidBase extends Base {
    ...
    protected AndroidDriver<MobileElement> androidDriver = (AndroidDriver<MobileElement>) Base.appiumDriver;
    ...
    public void tapOnElement(MobileItem mobileItem) {
        try {
            androidDriver.findElement(mobileItem.locatorType.toString(), mobileItem.locator).click();
        } catch (Exception e) {
    ...
    public MobileElement getElement(MobileItem mobileItem) {
        try {
            return androidDriver.findElement(mobileItem.locatorType.toString(), mobileItem.locator);
        } catch (Exception e) {
    ...
    public List<MobileElement> getElements(String screenName, LocatorType locatorType, String locator) {
        try {
            return androidDriver.findElements(locatorType.toString(), locator);
        } catch (Exception e) {
    ...
    private void waitElement(MobileItem mobileItem) {
        try {
            androidDriver.findElement(mobileItem.locatorType.toString(), mobileItem.locator);
        } catch (Exception e) {
    ...
    public boolean isExist(MobileItem mobileItem) {
        try {
            androidDriver.manage().timeouts().implicitlyWait(TestsUtility.TIMEOUT_EXIST_METHODS_MS, TimeUnit.MILLISECONDS);
            androidDriver.findElement(mobileItem.locatorType.toString(), mobileItem.locator);
            androidDriver.manage().timeouts().implicitlyWait(TestsUtility.TIMEOUT_DEFAULT_SEC, TimeUnit.SECONDS);
            return true;
        } catch (Exception e) {
            androidDriver.manage().timeouts().implicitlyWait(TestsUtility.TIMEOUT_DEFAULT_SEC, TimeUnit.SECONDS);
            return false;
    ...
    public boolean isExist(MobileItem mobileItem, int waitingMilliseconds) {
        try {
            androidDriver.manage().timeouts().implicitlyWait(waitingMilliseconds, TimeUnit.MILLISECONDS);
            androidDriver.findElement(mobileItem.locatorType.toString(), mobileItem.locator);
            androidDriver.manage().timeouts().implicitlyWait(TestsUtility.TIMEOUT_DEFAULT_SEC, TimeUnit.SECONDS);
            return true;
        } catch (Exception e) {
    ...
    protected void verifyElementsPresent(MobileItem scrollableArea, List<MobileItem> items, boolean startFromTop) {
        ...
        if (items.size() > 0) {
            ...
            logger.info(androidDriver.getPageSource()); // Может помочь для дебага упавших тестов.
        }
    }
    ...
    public void closeKeyboardIfPresent() {
        if (androidDriver.isKeyboardShown()) {
            ...
            androidDriver.pressKey(key.withKey(AndroidKey.BACK));
        }
    }
    ...
    public void swipeScreenByCoordinates(double startY, double endY, long swipeSpeed) {
        int startPoint = (int) (androidDriver.manage().window().getSize().height * startY); // Точка начала свайпа.

        int endPoint = (int) (androidDriver.manage().window().getSize().height * endY); // Точка конца свайпа.
        int width = androidDriver.manage().window().getSize().width; // Ширина дисплея.
    ...
    private void swipeTo(MobileItem scrollableArea, ScrollDirection direction) {
        ...
        androidDriver.executeScript("mobile: swipeGesture", swipe);
    }
